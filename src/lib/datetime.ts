...
// "today HH.MM" / "yesterday HH.MM" / "N days ago at HH.MM" — relative to
// now in Jakarta calendar days, time always rendered in Jakarta TZ.
export function formatRelativeJakarta(
  ...
  if (todayKey === thenKey) return `today ${time}`;
  ...
  if (diffDays > 1) return `${diffDays} days ago at ${time}`;
