...
export function TrackLinks({ track }: Props) {
  const artists = JSON.parse(track.artists) as string[];
  const query = encodeURIComponent(`${track.title} ${artists[0] ?? ""}`.trim());

  const links: { label: string; href: string; Icon: (p: { className?: string }) => JSX.Element }[] = [
    {
      label: "Lyrics on Genius",
      href: `https://genius.com/search?q=${query}`,
      Icon: GeniusIcon,
    },
    {
      label: "Apple Music",
    ...
    <div className="flex shrink-0 items-center gap-0.5">
      {links.map(({ label, href, Icon }) => (
        <a
          ...
          aria-label={label}
          title={label}
          className="p-2 text-neutral-500 hover:text-black dark:text-neutral-400 dark:hover:text-white"
        >
...
function GeniusIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className}>
